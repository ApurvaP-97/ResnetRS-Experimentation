...
def preprocess_image(preprocess_config):
    ...
    """
    ### YOUR CODE HERE
    '''Image Normalization is done here'''
    ...
    if preprocess_config['cutout']:
        ...
        preprocess_train.transforms.append(Cutout(num_holes=c_holes, dim=c_length))
...
class AutoAugmentCIFAR(object):
    """
    Refer to Report Section 5.2.(a). This is AutoAugment subpolicies for CIFAR10 dataset
...
class Cutout(object):
    """
    Implementing square cutouts. Refer to Report Section 5.2.(b)
    """
    def __init__(self, num_holes, dim):
        self.num_holes = num_holes
        self.dim = dim

    def __call__(self, image):
        """
        num_holes of dimension dim x dim cut out of our image
        """
        ...
        for n in range(self.num_holes):
            ...
            y1 = np.clip(y - self.dim // 2, 0, image.size(1))
            y2 = np.clip(y + self.dim // 2, 0, image.size(1))
            x1 = np.clip(x - self.dim // 2, 0, image.size(2))
            x2 = np.clip(x + self.dim // 2, 0, image.size(2))
